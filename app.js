...
// =====================================================
// DEFAULTS (devem vir ANTES de initUserData!)
// =====================================================
...
async function initUserData(uid) {
    ...
    if (!userDoc.exists()) {
        // Novo usuário: cria documento com defaults
        console.log("Novo usuário: criando dados padrão...");
        ...
        // Usuário existente: verifica se categorias estão vazias
        const existingData = userDoc.data();
        if (!existingData.categories || existingData.categories.length === 0) {
            console.log("Categorias vazias detectadas. Restaurando padrões...");
            await updateDoc(userRef, {
                categories: defaultCategories,
                subcategories: defaultSubcategories
            });
    ...
    // Listener em tempo real (dados já garantidos acima)
...
// Função global de emergência (acessível pelo console do navegador)
window.fixCategories = async () => {
    if (!currentUser) { alert('Faça login primeiro.'); return; }
    const userRef = doc(db, 'users', currentUser.uid);
    categories = [...defaultCategories];
    subcategories = [...defaultSubcategories];
    await updateDoc(userRef, { categories, subcategories });
    updateUI();
